...
* Incoming traffic to a local workload. which means traffic is received from a local host endpoint (e.g. eth0) and forwarded to a local workload(container/pod/vm). In this case the destination IP is the workload's IP and the host endpoint is just an intermediate hop. Ingress rules of the incoming host endpoint (eth0) applies to the traffic.
* Outgoing traffic from local workload. Which means traffic is coming from a workload and forwarded to a host endpoint (eth1). Egress rules of the host endpoint (eth1) applies to the traffic.

2. Traffic between host endpoints.
* Traffic from one host endpoint (eth0) to another host endpoint (eth1). In this case, Ingress rules of eth0 and egress rules of eth1 applies to the traffic.
...
Create a host endpoint for eth0 on host node1. Once host endpoint been created, packets goes into a local process from eth0 and traffic from local process to eth0 will been dropped by default deny rules. However, forwarded traffic to/from eth0 can goes through.
```
...
Create an empty policy to deny forwarded traffic for all host endpoints.
```
calicoctl apply -f - <<EOF
- apiVersion: v1
  kind: policy
