...
int multiple_tail_calls_test()
{
    ...
    const char* file_name ="c:\\k\\percpu.o";
    ...
    int map_fd = bpf_object__find_map_fd_by_name(object, "jump_map");
	if (map_fd < 0) {
		return -7;
	}

    // Store callee at index 0.
    index = 0;
    ...
    struct bpf_link* link = bpf_program__attach_xdp(caller, 17);
    if (link == nullptr) {
		return -10;
    }
